-- Q7. Find the names of schools (public or charter!) in the
-- Cambridge school district. 
-- Keep in mind that Cambridge, the city, contains a few school
-- districts, but DESE is interested in the district whose name
-- is “Cambridge.” 
--
--  The "schools" table contains the following columns:
--      "id", "district_id", "name", "type", "city", 'state' and "zip".
--
--  The "districts" table contains the following columns:
--      "id", "name", "type", "city", "state" and "zip".
-- 
-- Answer:
SELECT "name" FROM "schools"
WHERE "district_id" IN (
    SELECT "id" FROM "districts"
    WHERE "name" = 'Cambridge');

-- Result: OK
-- - 1 column and 17 rows.